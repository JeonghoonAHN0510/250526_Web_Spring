...
CREATE TABLE pointlog (
    plno      INT AUTO_INCREMENT,		-- 지급로그
    mno       INT NOT NULL,				-- 회원번호
    plpoint   INT NOT NULL,				-- 지급 포인트
    plcomment VARCHAR(100) NOT NULL,	-- 지급 사유
...
-- ---------------------- post ---------------------------------- --
create table post(
	pno         int auto_increment,			-- 게시물 번호
	ptitle       varchar(1000) not null ,	-- 게시물 제목
	pcontent   longtext   not null ,		-- 게시물 내용
	pdate       datetime default now() ,	-- 게시물 작성일
	pview      int default 0 ,				-- 게시물 조회수
	mno         int , -- 회원번호 fk			-- 게시물 작성자
	cno         int , -- 카테고리번호 fk		-- 게시물 카테고리 번호
	constraint primary key( pno ) ,
